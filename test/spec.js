...
module.exports = function(options) {
  ...
  before(function(done) {
    ...
    console.log('Is generating ' + 100 + ' entities...')
    let promise = Promise.resolve()
    let i = 1
    _.times(100, function() {
      const order = i++
      const group = order % 10
      promise = promise.then(function() {
        nextDay.setDate(nextDay.getDate() + 1)
        datesSaved.push(new Date(nextDay))
        return db.execute(
            'INSERT INTO person VALUES' + '($1, $2, $3, $4, $5, $6, $7)',
            [
  ...
  describe('querying', function() {
    it('should read all the records', function(done) {
      db.query(sv.build('SELECT * FROM ' + db.wrap('person')).statement)
    ...
    it('should read 10 records', function(done) {
      const view = sv.build('SELECT * FROM ' + db.wrap('person'), {
        where: {
          ENDERECO: '0'
        }
      })
    ...
    it('should read none record using and', function(done) {
      const view = sv.build('SELECT * FROM ' + db.wrap('person'), {
        where: {
          and: [{NUMERO: 'QRYTST'}, {NOMECAD: 'QUALQUER'}]
        }
      })
      db.query(view.statement, view.params)
          .then(function(recordset) {
    ...
    it('should read 10 records using or', function(done) {
      const view = sv.build('SELECT * FROM ' + db.wrap('person'), {
        where: {
          NUMERO: 'QRYTST',
          or: [{ENDERECO: '0'}, {NOMECAD: 'QUALQUER'}, {DATNASC: null}]
        }
      })
      db.query(view.statement, view.params)
          .then(function(recordset) {
    ...
    it('should throw a comparator error message', function(done) {
      try {
        const v = sv.build('SELECT * FROM ' + db.wrap('person'), {
          where: {
            NOMECAD: {
              xxx: '001'
            }
  ...
  describe('querying date and time', function() {
    it('should read 1 record in a date column', function(done) {
      ...
      db.query(
          view.statement,
          view.params.map(function(param) {
